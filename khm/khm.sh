...
case $1 in 
jym|1)
	echo '1,安装 gcc pcre-devel openssl-devel 需要的依赖包
2,创建一个用户
3,解源码包 tar
4,./configure 脚本安装源码包
...
fwml|3)
echo '/usr/local/nginx/sbin/nginx                    //启动服务
/usr/local/nginx/sbin/nginx -s stop            //关闭服务
 /usr/local/nginx/sbin/nginx -s reload        //重新加载配置文件
/usr/local/nginx/sbin/nginx -V           //查看软件信息
ln -s /usr/local/nginx/sbin/nginx /sbin/        //制作软链接 ,方便后期使用';;
ns|4)
echo 'netstat命令可以查看系统中启动的端口信息，该命令常用选项如下：
-a显示所有端口的信息
-n以数字格式显示端口号
-t显示TCP连接的端口
-u显示UDP连接的端口
...
lnmp|7)
echo "LNMP由 Linux nginx mariadb php 组成
需要的软件包 mariadb(数据库) mariadb-server(数据库服务端)mariadb-devel(数据库依赖包) 数据库端口为:3306
php(解释器)php-mysql(扩展包)php-fpm(服务) php-fpm服务端口为:9000 ";;
dtwz1|8)
echo '实现动态网站查看:
一:查看php-fpm配置文件
...
echo '三:创建php页面,测试LNPM架构是否解析PHP页面
...
dzcx|11)
echo '1,修改配置文件添加
rewrite /a.html  /b.html;   //访问a.html时会跳转到b.html网页
rewrite  ^/  http://www.tmooc.cn/;   //访问本机网站时会跳转到www.tmooc.cn网站
rewrite  ^/(.*) http://www.tmooc.cn/$1; //访问本机网站/下面子页面,会跳转到www.tmooc.cn/下相同的页面 

if ($http_user_agent ~* firefox ){	实现用Firefox浏览器阅览时会
rewrite /(.*) /firefox/$1;		转到Firefox专业的页面
}       				 

2,重新加载nginx服务,再用客户端认证
...
重新加载nginx服务,测试时会记录ip后会访问相同的web服务器';;
...
4)优化后再用ab高并发测试 ab -n 2000 -c 2000 http://192.168.4.5';;
yhbt|15)
echo '优化Nginx数据包头缓存
...
(注意访问是要添加8080端口)';;
ymtz3|25)
echo '/usr/local/tomcat/conf/server.xml配置文件
...
grevpn|40)
echo '步骤一：启用GRE模块（client和proxy都需要操作）
1）查看计算机当前加载的模块
lsmod                      //显示模块列表
...
jbjy|53)
echo '扩展知识：默认LVS不带健康检查功能，需要自己手动编写动态检测脚本，实现该功能：(参考脚本如下，仅供参考)
vim check.sh
#!/bin/bash
VIP=192.168.4.15:80
RIP1=192.168.4.100
...
|jym 	1	 为源码包的安装 
|bc 	2  	 报错的解释
|fwml 	3	 为nginx的使用命令的解析
|ns 	4	 netstat查看端口信息的选项 
|sj 	5	 升级nginx的步骤和注意事项
|yhrz 	6	 用户认证的步骤
|lnmp	7	 LNMP的解析
|dtwz为lnmp动态网站的构建:
	8 	(dtwz1)是php-fmp配置文件的样板
 	9	(dtwz2)是修改nginx配置文件的样板
 	10	(dtwz3)是测试LNMP构建是否能解析PHP网页
|dzcx 	11	 是nginx实现地址重写
|fxdl	12	配置nginx服务器,添加服务池,实现反向代理
|jqcsx	13	配置upstreamf服务器集群池属性
nginx优化 
	14	|yhpf		优化nginx的并发量
	15	|yhbt		优化nginx数据包头缓存
	16	|hcjtsj		浏览器本地缓存静态数据
	17	|rzqg		日志切割
	18	|ymys		对页面进行压缩处理
	19	|nchc		服务器内存缓存
|psgx	20	在后端LNMP服务器上部署Session共享
|tomcat 21	部署tomcat服务器软件
	22	|xnzj		配置服务器虚拟主机
	23	|ymtz		修改www.b.com网站的首页目录为base
	24	|ymtz2		页面跳转
	25	|ymtz3		页面跳转配置说明
	26	|tomssl		配置tomcat支持ssl加密网站
|hcfwq	27	配置varnish缓存服务器
|qtcz	28	其他操作
|git基本操作	
	29	|gitfwbs		部署git服务器
	30	|gitkhbs		git客户端测试
	...
	34	|fzct			解决版本分支的冲突问题
|git服务器形式	
	36	|gitssh			SSH协议服务器（支持读写操作）
	35	|git			Git协议服务器（只读操作的服务器）
	37	|githttp		HTTP协议服务器(只读操作的服务器)
	38	|zzrpm			制作nginx的RPM包
	39	|pzrpm			使用配置文件创建RPM包
|VPN模式	
	40	|grevpn			配置GREVPN
	41	|pptpvpn		创建PPTP VPN
	42	|ipsec			创建L2TP+IPSec VPN
|集群		
	43	|jq			什么是集群
	44	|jqmd			集群目的
	45	|jqfl			集群的分类
	...
	49	|ddsf			负载均衡调度算法
	50	|mlyf			ipvsadm用法
	51	|natjq			部署LVS-NAT集群
	52	|drjq			部署LVS-DR集群
	53	|jbjy			篇写脚本检验lvs健康状况'		
esac
